...
$logFile = '/home/felis/www/scripts/logfile.txt';


if (!isset($startUrl))
{
    exit('Parameter --base is not set' . "\n");
}
if (!isset($logFile))
{
    $logFile = dirname(__FILE__) . DIRECTORY_SEPARATOR . 'urlparser.log';
}

$startUrl = parse_url($startUrl, PHP_URL_HOST);
if ($startUrl) $startUrl = 'http://' . $startUrl . '/';

if (!$startUrl) exit('Invalid --base argument' . "\n");

if (!is_writable($logFile)) exit('Invalid --to argument. Access denided' . "\n");
...
class UrlParser
{
    ...
    {
        $this->_parseLinks($this->startUrl);
    }

    public function _parseLinks($url, $stop = false)
    {
        $headers = get_headers($url, 1);
        if (mb_strpos($headers[0], '404') !== false) return array();
        if (!is_array($headers['Content-Type']))
        {
            if (mb_strpos($headers['Content-Type'], 'text/html') === false) return array();
        }
        else
        {
            if (mb_strpos($headers['Content-Type'][0], 'text/html') === false) return array();
        }
        echo $url . "\n";
        ...
        if (is_array($matches[1]) && count($matches[1]))
        {
            ...
            foreach ($results as $k => &$url)
            {
                if (!preg_match('%^(http|https)://%u', $url) || 
                    mb_strpos($url, $this->startUrl) === 0)
                {
                    $sections = parse_url($url);
                    if (isset($sections['scheme']) &&
                        ($sections['scheme'] != 'http' && $sections['scheme'] != 'https'))
                        {
                            //echo 'Invalid scheme ' . $url . "\n";
                            unset($results[$k]);
							$this->checked[] = $url;
                            continue;
                        }
                    ...
                    $url = trim($this->startUrl . ltrim($url, '/'));
					
					if (in_array(trim($url), (array)$this->checked))
					{
						//echo 'Page already parsed ' . $url . "\n";
						unset($results[$k]);
						continue;
					}
echo $url."\n";
...
crawl($url,$link,$user,$password);
                    ...
                    $headers = get_headers($url, 1);
                    if (mb_strpos($headers[0], '404') !== false)
                    {
                    ...
                    if (!is_array($headers['Content-Type']))
                    {
                        if (mb_strpos($headers['Content-Type'], 'text/html') === false)
                        {
                            //echo 'It is file ' . $url . "\n";
                            unset($results[$k]);
							$this->checked[] = $url;
                    ...
                    {
                        if (mb_strpos($headers['Content-Type'][0], 'text/html') === false)
                        {
                            //echo 'It is file ' . $url . "\n";
                            unset($results[$k]);
							$this->checked[] = $url;
			...
			while ($url = array_shift($results))
			{
				if (in_array(trim($url), (array)$this->results))
                {
                    //echo 'Page already parsed ' . $url . "\n";
                    continue;
                }
                $this->results[] = $url;
                $subResults = array_unique((array)$this->_parseLinks($url, true));
                $results = array_unique(array_merge($results, $subResults));
			}
        }
    }
...
$parser = new UrlParser($startUrl);
$parser->parseLinks();
$links = array_unique($parser->getLinks());
sort($links);
file_put_contents($logFile, implode("\n", $links));


$url='http://www.kinopoisk.ru/film/229069/votes/'."\n";
...
if ($id!=false){
insert_row($id,$link,$user,$password);
}
}
}
}
